...
// TODO: add data channel for commands.
...
func NewSubscriber(id string, cfg WebRTCTransportConfig) (*Subscriber, error) {
	...
	pc.OnICEConnectionStateChange(func(connectionState webrtc.ICEConnectionState) {
		...
		case webrtc.ICEConnectionStateClosed:
			s.closeOnce.Do(func() {
				Logger.Info("webrtc ice closed", "peer_id", s.id)
...
func (s *Subscriber) OnNegotiationNeeded(f func()) {
	// 重协商至少是250ms..
	debounced := helper.New(250 * time.Millisecond)
	s.negotiate = func() {
		// negotiate的回调函数.
		debounced(f)
	}
...
// send SourceDescription rtcp packets to all peers.
